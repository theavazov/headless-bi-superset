...
type ChartResponseData = {
    result: Array<{ data?: unknown[]; }>;
};

export function BN_POS_Modal(props: ModalPropsType) {
        ...
        const loadData = async (): Promise<void> => {
            try {
                const [
                    ...
                    getChartResults<BigNumberTrendlineRow[][]>(CHART_IDS.trend)
                ]);

                const getChartData = <T,>(response: ChartResponseData): T[] => {
                    return ((response.result.at(0)?.data as T[]) ?? []);
                };

                const getLatestCount = (response: ChartResponseData): number => {
                    const data = getChartData<BigNumberTrendlineRow>(response);

                    if (!data.length) return 0

                    return Number(data.at(-1)?.count ?? 0);
                };

                setConnected(getLatestCount(connectedResponse));
                setActive(getLatestCount(activeResponse));
                setNewPerMonth(getLatestCount(newPerMonthResponse));
                setInactive90Days(getLatestCount(inactiveResponse));
                setTrendData(getFilteredTrend(getChartData<TrendData>(trendResponse)));
                setTrends({
                ...
                console.error("Failed to load POS modal data:", error);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const option = {
        ...
        xAxis: {
            ...
            axisLabel: {
                ...
                /*
                 * Prevent every single month from becoming
                 * overcrowded.
                 */
                interval: 1,
            },
