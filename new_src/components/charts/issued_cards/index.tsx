...
import type { IssuedMonthlyCardData } from '../../../api/types';
...
export const IssuedCardsChart = ({ props }: { props: IBarChart }) => {
    ...
    useEffect(() => {
        const loadData = async (): Promise<void> => {
            try {
                const obj = await getChartResults(410);

                const issuedData = obj.result[0]?.data as IssuedMonthlyCardData[];
                ...
                const filteredIssuedData = issuedData.filter((row) => {
                    const reportMonth = Number(row.ISSUE_DATE);

                    return reportMonth >= startDate && reportMonth < endDate;
                });

                const months = filteredIssuedData.map((row) =>
                    formatMonth(Number(row.ISSUE_DATE))
                );

                const cards = filteredIssuedData.map(
                    (row) => Number(row.number_of_cards)
                );
            ...
            areaStyle: {
                color: {
                    ...
                    colorStops: [
                        {
                            offset: 0,
                            color: 'rgba(16, 166, 106, 0.20)',
                        },
                        {
                            offset: 1,
                            color: 'rgba(16, 166, 106, 0.04)',
                        },
                    ],
                },
            },
